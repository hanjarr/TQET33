...
class Utils:
    ...
    def __init__(self, directory, target, search_size, extension, poi='T9'):
        ...
        self._target_length = self._water_target.patient_size*1000

        ''' Target poi ground truth'''
        self._target_poi = np.array(self._water_target.get_poi(poi))
        self._T9_poi = np.array(self._water_target.get_poi('T9'))
    ...
    def extract_ground_truth(self, reduced_mask):
        ...
        ''' Numpy array of size Nx3 where N = number of voxels in reduces space'''
        euclidean_dist = np.transpose(np.array([dist[reduced_mask]*self._target_voxel_size[ind] for ind, dist in enumerate(voxel_dist)]))
