...
class Module:
    ...
    def pre_selection(self):
        ...
        selection_regressor = Regression(self._nbr_of_trees_select, self._max_features_select, self._bootstrap)
        ...
        for target in self._selection_targets:
            ...
            ''' Select best filters '''
            selection_regressor.generate_estimator(extracted_features, ground_truth)

        ''' Extract best filters '''
        selection = selection_regressor.feature_selection(self._selected_filters)
    ...
    def training(self, filter_bank, filter_parameters):
        ...
        ''' Create regression class object '''
        train_regressor = Regression(self._nbr_of_trees, self._max_features, self._bootstrap)
        ...
        for target in self._train_targets:
            ...
            train_regressor.generate_estimator(extracted_features, ground_truth)

        ''' Save forest to file'''
        joblib.dump(train_regressor, 'RegressionForest.pkl', compress=1)

        return train_regressor

    def testing(self, regressor, filter_bank, filter_parameters):
        ...
        for target in self._test_targets:
            ...
            regression = run_estimator(regressor, test_features)

            reg_poi = utils.regression_voting(regression, position_grids)
            print(reg_poi)
