...
class Utils:
    ...
    def extract_ground_truth(self, reduced_mask):
        ...
        reduced_voxel_dist = [np.reshape(dist[reduced_mask],(self._reduced_size*2+1)) for dist in voxel_dist]
    ...
    def extract_grids(self, reduced_mask):
        ...
        reduced_grids = [np.reshape(grid[reduced_mask],(self._reduced_size*2+1)) for grid in grids]

        position_vectors = [reduced_grids[0][:,0,0], reduced_grids[1][0,:,0], reduced_grids[2][0,0,:]]

        return position_vectors

    def regression_voting(self, regressions, position_vectors):


        voting = np.array(position_vectors) - np.array(regressions)

        print(np.shape(voting[0]))
